...
#define BD_NOT_CONNECTED	2
	...
	void RegisterHooks()
	{
		Live_IsUserSignedInToDemonware_Hook.Create(0x82595B98, Live_IsUserSignedInToDemonware);
		LiveStorage_DoWeHaveFFOTD_Hook.Create(0x825D5FE0, LiveStorage_DoWeHaveFFOTD);
		LiveStorage_ValidateFFOTD_Hook.Create(0x825D6010, LiveStorage_ValidateFFOTD);
		LiveStorage_DoWeHaveAllStats_Hook.Create(0x825CC2E0, LiveStorage_DoWeHaveAllStats);
		LiveStorage_DoWeHavePlaylists_Hook.Create(0x825D5668, LiveStorage_DoWeHavePlaylists);
		LiveStorage_DoWeHaveLeagues_Hook.Create(0x825D5698, LiveStorage_DoWeHaveLeagues);
		LiveStorage_IsTimeSynced_Hook.Create(0x825CD438, LiveStorage_IsTimeSynced);
		LiveStorage_DoWeHaveContracts_Hook.Create(0x825D5678, LiveStorage_DoWeHaveContracts);
		Utils::Hook::SetValue<uint8_t>(0x841DECA9, 1); // s_geoLocationRetrieved
		LiveElite_CheckProgress_Hook.Create(0x825A0D88, LiveElite_CheckProgress);
		Live_IsLspCacheInited_Hook.Create(0x825DD640, Live_IsLspCacheInited);
		LiveStorage_DoWeHaveStats_Hook.Create(0x825CC288, LiveStorage_DoWeHaveStats);
	}
