...
#define BD_NOT_CONNECTED	2
	...
	void RegisterHooks()
	{
		Live_IsUserSignedInToDemonware_Hook.Create(0x8259C0B0, Live_IsUserSignedInToDemonware);
		LiveStorage_DoWeHaveFFOTD_Hook.Create(0x825DC788, LiveStorage_DoWeHaveFFOTD);
		LiveStorage_ValidateFFOTD_Hook.Create(0x825DC7B8, LiveStorage_ValidateFFOTD);
		LiveStorage_DoWeHaveAllStats_Hook.Create(0x825D2D58, LiveStorage_DoWeHaveAllStats);
		LiveStorage_DoWeHavePlaylists_Hook.Create(0x825DBE30, LiveStorage_DoWeHavePlaylists);
		LiveStorage_DoWeHaveLeagues_Hook.Create(0x825DBE60, LiveStorage_DoWeHaveLeagues);
		LiveStorage_IsTimeSynced_Hook.Create(0x825D3E70, LiveStorage_IsTimeSynced);
		LiveStorage_DoWeHaveContracts_Hook.Create(0x825DBE40, LiveStorage_DoWeHaveContracts);
		Utils::Hook::SetValue<uint8_t>(0x841EE821, 1); // s_geoLocationRetrieved
		LiveElite_CheckProgress_Hook.Create(0x825A7260, LiveElite_CheckProgress);
		Live_IsLspCacheInited_Hook.Create(0x825E3D60, Live_IsLspCacheInited);
		LiveStorage_DoWeHaveStats_Hook.Create(0x825D2D00, LiveStorage_DoWeHaveStats);
	}
